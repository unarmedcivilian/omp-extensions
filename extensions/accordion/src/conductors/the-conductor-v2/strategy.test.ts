...
import { createAccordionState, computeFoldPlan, liveTokensAtLevels, contentForLevel, foldCode } from "./strategy.ts";
...
test("custom trim and digest content carries agent recovery tags", () => {
	const block = viewToParsed([
		{
			id: "m1:p0",
			kind: "text",
			turn: 1,
			...
			protected: false,
			grouped: false,
			text: "deploy decision FINAL_MARKER=MANGO-WHISPER-9\n".repeat(80),
		},
	]).blocks[0];
	const state = createAccordionState();
	const trim = contentForLevel(block, 1, state, {}, undefined, "deploy decision");
	const digest = contentForLevel(block, 2, state, {}, undefined, "deploy decision");
	assert.match(trim, new RegExp(`^\\{#${foldCode(block.id)} FOLDED\\}`));
	assert.match(digest, new RegExp(`^\\{#${foldCode(block.id)} FOLDED\\}`));
});

test("custom group digest carries the group recovery tag", () => {
	const block = viewToParsed([
		{
			id: "m1:p0",
			kind: "text",
			...
			protected: false,
			grouped: false,
			text: "groupable content ".repeat(100),
		},
	]).blocks[0];
	const state = createAccordionState();
	const content = contentForLevel(
		block,
		2,
		state,
		{},
		{ blockIds: [block.id, "m2:p0", "m3:p0"], firstTurn: 1, lastTurn: 3, members: 3 },
		"continue",
	);
	assert.match(content, new RegExp(`^\\{#${foldCode(`g:${block.id}`)} FOLDED\\}`));
});
